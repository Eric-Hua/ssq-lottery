...
| 功能 | 说明 |
| --- | --- |
| 🎯 智能选号 | 单式 / 复式 / 胆拖三种玩法选号板 + 机选一注,实时校验并计算注数,可一键复制;支持按「重号/邻号形态」约束推荐与机选 |
...
| 📜 历史开奖 | 每期展示完整 1–33 红球(开出标红/未开置灰,金圈=重号、青圈=邻号)+ 蓝球,期号搜索、分页、排序 |
| 📊 数据统计 | 红蓝球频次图、当前遗漏图、最近 30 期和值走势、热号/冷号 TOP 榜(近10/30/50/500 期可切) |
| 🧮 号码区间对照 | 每行一张 1–33 网格:我的自选(可编辑/可隐藏)、最多 5 条推荐柱(可增删)、近 10 期开奖;带三区比 |
...
## 号码区间对照(自选 / 推荐 vs 近 10 期)

「🎯 智能选号」页底部有一块对照面板:**每行都是完整的 1–33 红球网格**,红色=选中/开出,灰色=未选中/未开出。

| 行 | 含义 | 可否编辑 |
| --- | --- | --- |
| **我的自选**(金色标题) | 当前选号板已选的红球,点选时**实时更新** | 点红球可直接删除该号;行尾 ✕ 隐藏整行(工具栏可加回) |
| **推荐 1~5**(青色标题) | 从「推荐方案」加入的柱(单条「加入对照」,或一键「把 5 注推荐加入」) | 最多 **5 条**;行尾 ✕ 删除;工具栏可「清空推荐柱」 |
| 第 N 期 × 10 行 | 近 10 期开奖号码(最新在前) | — |

- 网格在 **11/12、22/23 之间留白**,直接看出一区(1–11)/ 二区(12–22)/ 三区(23–33)的分布
- 每行最右列为**三区比**(如 `2 : 3 : 1`),便于横向比对

> 用处:不用逐期翻历史,一眼看出自选/推荐号码与近期开奖的区间分布差多少 —— 比如近 10 期都是二区偏多,而你的号全挤在一区。

## 重号 / 邻号形态选号

「🎯 智能选号」页顶部可选择形态约束(作用于**推荐方案**与**随机生成**):

- **重号** = 与上期相同的号码;**邻号** = 与上期某个号差 ±1(与重号互斥);**关联数** = 重号 + 邻号
- 可选区间:不限 / **2–4(推荐)** / 1–3 / 0–2 / 3–5 / 4–6
- 近 50 期实测 + 30 万次随机模拟:关联数落在 **2–4 个** 的期数约占 80%;0 个与 6 个都很罕见(约 2% / 0.6%)
- 历史开奖页同样用**金圈标重号、青圈标邻号**,方便对照

> ⚠️ 形态约束只是让号码“长得像真实开奖”(同时自然避开上期全无关这种罕见形态),**它减少的是你买的注数,不是风险**;每注中奖概率不变。
