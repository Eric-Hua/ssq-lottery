...
(() => {
  ...
  function freqRedBefore(t, w, n) { var f = t - w; return prefRed[n][t] - (f > 0 ? prefRed[n][f] : 0); }
  function freqBlueBefore(t, w, n) { var f = t - w; return prefBlue[n][t] - (f > 0 ? prefBlue[n][f] : 0); }

  function consecRuns(reds) {
    var runs = [], cur = [];
    for (var i = 0; i < reds.length; i++) {
      if (!cur.length) { cur = [reds[i]]; }
      else if (reds[i] === cur[cur.length - 1] + 1) { cur.push(reds[i]); }
      else { if (cur.length >= 2) runs.push(cur.slice()); cur = [reds[i]]; }
    }
  ...
  /* ---------------- 号码小工具(用户公式用) ---------------- */
  function digitSum(v) {
    var s = String(Math.abs(Math.trunc(v))), out = 0;
    for (var i = 0; i < s.length; i++) out += Number(s[i]);
    return out;
  }
  function lastDigit(v) { return Math.abs(Math.trunc(v)) % 10; }
  function toBlueUnit(v) { var d = lastDigit(v); return d === 0 ? 10 : d; }
  function tailBlues(digit) { return ALL_B.filter((n) => n % 10 === digit); }
  function mirrorSorted(t) {
    return prevOf(t).red.map((n) => 34 - n).sort((a, b) => a - b);
  }
  ...
  function mkSite() {
    var F = [];
    function add(kind, name, desc, def, kill) { F.push({ src: "本站", kind: kind, name: name, desc: desc, def: def, kill: kill }); }

    add("red", "红球和值杀号", "上期红球和值 mod 33", true, (c) => [m33(c.prev.red.reduce((a, b) => a + b, 0))]);
    add("red", "首尾和值杀号", "上期首红+尾红 mod 33", true, (c) => [m33(c.prev.red[0] + c.prev.red[5])]);
    add("red", "跨度杀号", "上期跨度 mod 33", true, (c) => [m33(c.prev.red[5] - c.prev.red[0])]);
    add("red", "奇偶和差杀号", "上期(奇数和−偶数和) mod 33", true, (c) => {
      var odd = 0, even = 0;
      c.prev.red.forEach((n) => { if (n % 2) odd += n; else even += n; });
    ...
    add("red", "蓝球+5杀红", "上期蓝球+5 mod 33", true, (c) => [m33(c.prev.blue + 5)]);
    add("red", "蓝球×2杀红", "上期蓝球×2 mod 33", true, (c) => [m33(c.prev.blue * 2)]);
    add("red", "上期蓝球作红杀", "杀上期蓝球那个号", true, (c) => [c.prev.blue]);
    add("red", "连号端点杀号", "上期连号区间的首尾号", true, (c) => {
      var out = [];
      consecRuns(c.prev.red).forEach((run) => { out.push(run[0], run[run.length - 1]); });
      return out;
    });
    add("red", "超冷号杀号", "遗漏≥25期的红球", true, (c) => ALL_R.filter((n) => c.missR[n] >= 25));
    add("red", "过热号杀号", "近5期出现≥3次的红球", true, (c) => ALL_R.filter((n) => freqRedBefore(c.t, 5, n) >= 3));
    add("red", "同尾号杀号(激进)", "上期出现过的尾数 → 全部同尾号", false, (c) => {
      var tails = {};
      c.prev.red.forEach((n) => { tails[n % 10] = true; });
      return ALL_R.filter((n) => tails[n % 10]);
    });

    add("blue", "上期蓝球重杀", "杀上期蓝球", true, (c) => [c.prev.blue]);
    add("blue", "蓝球+1杀号", "上期蓝球+1 mod 16", true, (c) => [m16(c.prev.blue + 1)]);
    add("blue", "蓝球-1杀号", "上期蓝球−1 mod 16", true, (c) => [m16(c.prev.blue - 1)]);
    add("blue", "红球和值尾杀蓝", "蓝球个位 = 上期红球和值个位", true, (c) => {
      var tail = c.prev.red.reduce((a, b) => a + b, 0) % 10;
      return ALL_B.filter((n) => n % 10 === tail);
    });
    add("blue", "红球跨度杀蓝", "上期红球跨度 mod 16", true, (c) => [m16(c.prev.red[5] - c.prev.red[0])]);
    add("blue", "冷蓝球杀号", "遗漏≥20期的蓝球", true, (c) => ALL_B.filter((n) => c.missB[n] >= 20));
    add("blue", "热蓝球杀号(激进)", "近8期出现≥4次的蓝球", false, (c) => ALL_B.filter((n) => freqBlueBefore(c.t, 8, n) >= 4));
    return F;
  }

  function mkUser() {
    var F = [];
    function add(kind, name, desc, kill) { F.push({ src: "你提供", kind: kind, name: name, desc: desc, def: true, kill: kill }); }

    // —— 蓝球 5 大公式(⑤ 拆成三式)
    add("blue", "①A+B 绝杀法", "上上期蓝+上期蓝,取个位(0→10)", (c) => [toBlueUnit(c.prev2.blue + c.prev.blue)]);
    add("blue", "②A+16 绝杀法", "上期蓝+16,取个位(0→10)", (c) => [toBlueUnit(c.prev.blue + 16)]);
    add("blue", "③A+B+C 绝杀法", "上3期蓝球之和,取个位(0→10)", (c) => [toBlueUnit(c.prev3.blue + c.prev2.blue + c.prev.blue)]);
    add("blue", "④红球+蓝球绝杀法", "上期红球和+上期蓝,取个位(0→10)", (c) => [toBlueUnit(c.prev.red.reduce((a, b) => a + b, 0) + c.prev.blue)]);
    add("blue", "⑤-15 减蓝球杀尾", "15−上期蓝 → 杀该尾数", (c) => tailBlues(lastDigit(15 - c.prev.blue)));
    add("blue", "⑤-19 减蓝球杀尾", "19−上期蓝 → 杀该尾数", (c) => tailBlues(lastDigit(19 - c.prev.blue)));
    add("blue", "⑤-21 减蓝球杀尾", "21−上期蓝 → 杀该尾数", (c) => tailBlues(lastDigit(21 - c.prev.blue)));

    // —— 红球 6 大策略(⑥ 拆成两式)
    add("red", "①号码拆分相加杀号", "上期每个红球各自拆位相加", (c) => c.prev.red.map(digitSum));
    add("red", "②近期高频号杀号", "近5期出现≥3次(与本站「过热号」同式)", (c) => ALL_R.filter((n) => freqRedBefore(c.t, 5, n) >= 3));
    add("red", "③最值号码杀号", "上期最大号 + 最小号", (c) => [c.prev.red[0], c.prev.red[5]]);
    add("red", "④红蓝差值杀号", "上期最大红球 − 上期蓝球", (c) => {
      var v = c.prev.red[5] - c.prev.blue;
      return (v >= 1 && v <= 33) ? [v] : [];
    });
    add("red", "⑤和值拆分杀号", "上期红球和值拆位相加", (c) => [digitSum(c.prev.red.reduce((a, b) => a + b, 0))]);
    add("red", "⑥-1 对称码第3位+7", "对称码(红球关于17镜像 34−n)升序第3位 + 7", (c) => {
      var m = mirrorSorted(c.t);
      return [m[2] + 7];
    });
    add("red", "⑥-2 对称码第1位×0.88", "对称码升序第1位 × 0.88(向下取整)", (c) => {
      var m = mirrorSorted(c.t);
    ...
    for (t = WARMUP + 1; t < N; t++) {
      ...
      for (i = 0; i < formulas.length; i++) {
        ...
        for (var j = 0; j < arr.length; j++) {
          var n = arr[j];
          if (kind === "red") {
            if (n >= 1 && n <= 33 && !seen[n]) { seen[n] = true; killed.push(n); }
          } else if (n >= 1 && n <= 16 && !seen[n]) { seen[n] = true; killed.push(n); }
        }
      }
      ...
      acc.base += b;
      acc.wrong += wrong;
      if (wrong === 0) { acc.hit++; acc.bad = 0; }
      else { acc.bad++; if (acc.bad > acc.maxbad) acc.maxbad = acc.bad; }
    }
    return acc;
  }
  ...
  function makeRow(formulas, kind, label, desc, src) {
    ...
    var n = acc.n || 1;
    var a = acc.hit / n, b = acc.base / n;
    var p = acc.n ? binomTwoSided(acc.hit, acc.n, b) : 1;
    var ci = wilson(acc.hit, acc.n);
    var v = verdict(acc.n, a, b, p);
    return {
      name: label, desc: desc, src: src, kind: kind,
      n: acc.n, avgKill: acc.kill / n, hit: acc.hit, acc: a, base: b, p: p, ci: ci,
      avgWrong: acc.wrong / n, maxbad: acc.maxbad, verdict: v
    };
  }

  function runAll() {
    var site = mkSite(), user = mkUser();
    ...
    var rows = { red: [], blue: [] };
    var i;
    for (i = 0; i < siteRed.length; i++) rows.red.push(makeRow([siteRed[i]], "red", siteRed[i].name, siteRed[i].desc, "本站" + (siteRed[i].def ? "" : "·默认关")));
    rows.red.push(makeRow(siteRed.filter((f) => f.def), "red", "【本站默认 11 公式并集】", "页面默认勾选的范围", "本站"));
    for (i = 0; i < userRed.length; i++) rows.red.push(makeRow([userRed[i]], "red", userRed[i].name, userRed[i].desc, "你提供"));
    rows.red.push(makeRow(userRed, "red", "【你提供的 7 式全部并用】", "上表 7 条红球公式的并集", "你提供"));

    for (i = 0; i < siteBlue.length; i++) rows.blue.push(makeRow([siteBlue[i]], "blue", siteBlue[i].name, siteBlue[i].desc, "本站" + (siteBlue[i].def ? "" : "·默认关")));
    rows.blue.push(makeRow(siteBlue.filter((f) => f.def), "blue", "【本站默认 6 公式并集】", "页面默认勾选的范围", "本站"));
    for (i = 0; i < userBlue.length; i++) rows.blue.push(makeRow([userBlue[i]], "blue", userBlue[i].name, userBlue[i].desc, "你提供"));
    rows.blue.push(makeRow(userBlue, "blue", "【你提供的 7 式全部并用】", "上表 7 条蓝球公式的并集", "你提供"));
  ...
  function randomPanel(kind) {
    var accs = [], i, t, c, killed, actual, v, hit, n;
    var vals = () => Math.floor(Math.random() * 19) - 9;
    for (i = 0; i < MAX_ROWS_FOR_RANDOM; i++) {
      var a = vals(), b = vals(), cc = vals(), d = vals(), e = vals(), f = vals();
      hit = 0; n = 0;
      for (t = WARMUP + 1; t < N; t++) {
        c = DRAWS[t - 1];
        if (kind === "red") {
          ...
          v = a * sum + b * (c.red[5] - c.red[0]) + cc * c.red[0] + d * c.red[5] + e * c.blue + f;
          killed = [((v % 33) + 33) % 33 + 1];
          actual = DRAWS[t].red;
        } else {
          var s2 = 0;
  ...
  function pp(x) { var v = x * 100; return (v >= 0 ? "+" : "") + v.toFixed(1) + "pp"; }
  function pf(p) { return p < 0.001 ? "<0.001" : p.toFixed(3); }
  ...
  var HEADERS = ["公式", "来源", "参评期数", "平均杀号", "全杀对期数", "准确率", "随机基线", "实测−基线", "P 值", "结论"];

  function renderTable(tableEl, rows) {
    clearEl(tableEl);
    var thead = el("thead"), tr = el("tr"), i;
    for (i = 0; i < HEADERS.length; i++) tr.appendChild(el("th", null, HEADERS[i]));
  ...
  function renderSummary(rows) {
    ...
    var from = DATA[WARMUP + 1][0], to = DATA[N - 1][0];
    var tested = N - WARMUP - 1;
    var nSig = 0;
    rows.red.concat(rows.blue).forEach((r) => { if (r.verdict.cls === "vt-good" || r.verdict.cls === "vt-bad") nSig++; });

    var line1 = el("div", "rec-meta");
    line1.textContent = "回测区间:第 " + from + " ~ " + to + " 期,共 " + tested + " 期(前 " + WARMUP + " 期作历史预热);共检验 " + nTests + " 条公式(另附 " + (rows.red.length + rows.blue.length - nTests) + " 行并集,不计入校正)。";
    ...
    warn.textContent = "杀「1 个」号码的天然准确率:红球 81.8%、蓝球 93.8%(闭眼乱杀也是这个数)。所以要同时看「平均杀号个数」和「随机基线」——只看准确率没有意义。杀号越多,越容易踩中,准确率必然下降。";
    ...
    var passTxt = nSig === 0
      ? "本页所有公式都没有通过多重比较校正(Bonferroni 阈值 = " + thr + "):没有任何一条能证明优于随机。"
      : "有 " + nSig + " 项通过了 Bonferroni 校正(阈值 " + thr + ")。";
    line3.appendChild(el("span", "vtag " + (nSig === 0 ? "vt-neutral" : "vt-good"), nSig === 0 ? "结论:全部与随机无异" : "结论:有显著项"));
    var s3 = el("div", "model-note");
    s3.style.borderTop = "none";
    s3.style.marginTop = "8px";
    s3.style.paddingTop = "0";
    s3.textContent = passTxt + " P 值来自精确二项检验(双侧),用每期实际的杀号个数对应的随机基线做零假设。";
    ...
    rp.appendChild(load);
    window.setTimeout(() => {
      var red = randomPanel("red"), blue = randomPanel("blue");
      clearEl(rp);
      rp.appendChild(el("div", "rec-meta", "随机公式对照 —— 随机生成 " + red.n + " 个「每次都只杀 1 个号」的无意义公式(形如 (a×和值+b×跨度+c×首+d×尾+e×蓝+f) mod 33 + 1,系数随机),它们的准确率分布:"));
      var t = el("table", "hist");
      var th = el("thead"), htr = el("tr");
      ["项目", "红球(基线 81.8%)", "蓝球(基线 93.8%)"].forEach((x) => { htr.appendChild(el("th", null, x)); });
      th.appendChild(htr);
      t.appendChild(th);
      var tb = el("tbody");
      [["中位数", red.median, blue.median], ["90 分位", red.p90, blue.p90], ["95 分位", red.p95, blue.p95], ["最大值", red.max, blue.max]].forEach((line) => {
        var tr = el("tr");
      ...
      note.textContent = "含义:一堆完全随机的公式里,最高的一条也能达到 " + pct(red.max) + "(红)/" + pct(blue.max) + "(蓝)。所以「某公式准确率 85%」这种说法本身不构成任何证据 —— 它可能只是噪声里的幸运儿。";
      rp.appendChild(note);
    }, 30);
  }

  function toMarkdown(rows) {
    var out = ["| 公式 | 来源 | 参评期数 | 平均杀号个数 | 全部杀对期数 | 准确率 | 随机基线 | 实测−基线 | P 值 | 结论 |",
               "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |"];
    rows.forEach((r) => {
      out.push("| " + r.name + " | " + r.src + " | " + r.n + " | " + r.avgKill.toFixed(2) + " | " + r.hit +
        " | " + pct(r.acc) + " | " + pct(r.base) + " | " + pp(r.acc - r.base) + " | " + pf(r.p) + " | " + r.verdict.text + " |");
    });
    return out.join("\n");
  }
